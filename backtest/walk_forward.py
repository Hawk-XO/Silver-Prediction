...
def run_walk_forward(
    ...
    progress_callback: optional callable(done, total) invoked once per
        iteration of the fold loop (done = folds processed so far, total =
        folds in this run). Purely a UI hook (e.g. the Streamlit control
    ...
    n = len(df)
    y = df[target_col]
    total_folds = max(n - cfg.min_train_size, 1)
